...
/*   Updated: 2025/09/29 12:21:58 by thde-sou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    think(t_philo *philo)
{
    ...
    if (time_left <= philo->app->time_eat)
        return ;
    time_think = calcule_think(philo);
    if(time_think > 0)
    {
...
void    eat(t_philo *philo)
{
    ...
    if(philo->meals == 0 && philo->id % 2 == 0)
        precise_sleep(philo->app, philo->app->time_eat / 2);
    get_fork(philo);
    pthread_mutex_lock(&philo->app->m_meal);
    ...
    precise_sleep(philo->app, philo->app->time_eat);
    drop_fork(philo);
    set_satisfied(philo);
}
